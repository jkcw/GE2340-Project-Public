...
def train(n, heap_arr, update):
    ...
    for i in range(100 // 5 + 1):
        dict_time[i * 0.05] = False
        arr_time.append(round(i * 0.05, 5))

    for i in range(n):
        if round(i / n, 5) in arr_time:
            dict_time[round(i / n, 2)] = True
            value = round(i / n, 2) * 100
            print(str(value))
            update.message.reply_text("Loading: " + str(value) + "%")
